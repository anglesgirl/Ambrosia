...
Game_Event.prototype.chaseConditions = function(dis) {
	if (this._smellPlayer && $gameSwitches._data[434]){
		if (dis <= $gameVariables._data[2001]) return true;
	}
	if (dis <= this._chaseRange){
		if (this.nonSeePlayer()){
			this._alertLock = this._sightLock;
			return true;
		}
		if (this.canSeePlayer()) return true;
	}else{
		if (dis <= this._chaseRange * 2 && this._chasePlayer){
			if (this.nonSeePlayer()){
				this._alertLock = this._sightLock;
				return true;
			}
